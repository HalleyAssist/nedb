  ...
  this.db.getCandidates(this.query, (err, candidates) => {
    ...
    try {
      for(const doc of candidates){
        if (!model.match(doc, this.query)) continue
    ...
    if (this._sort) {
      ...
      res.sort((a, b) => {
        ...
        for (let i = 0; i < criteria.length; i++) {
          ...
          compare = criterion.direction * model.compareThings(model.getDotValue(a, criterion.key), model.getDotValue(b, criterion.key), this.db.compareStrings);
          if (compare !== 0) {
            return compare;
          }
        }
