...
function prepare (query, specific = true) {
  ...
  const fn = new Function('comparisonFunctions', 'logicalOperators', 'matchQueryPart', 'getDotValue', `return (query, obj)=>{${fnCode}}`);
  return fn(comparisonFunctions, logicalOperators, matchQueryPart, getDotValue)
}
