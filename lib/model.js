...
function checkKey (k, v) {
  ...
  if (k[0] === '$' && !(k === '$$date' && typeof v === 'number') && !(k === '$$deleted') && !(k === '$$indexCreated') && !(k === '$$indexRemoved')) {
...
function modify (obj, updateQuery) {
  ...
  if (updateQuery._id && updateQuery._id !== obj._id) { throw new Error("You cannot change a document's _id"); }
  ...
  if (!updateQueryDollar) {
    ...
    for(const m of modifiers) {
      ...
      if (!fn) { throw new Error("Unknown modifier " + m); }
...
/**
 * Prepare a function for a specific query
 * 
 * @param {Object} obj Document to check
 * @param {Object} query
 */
function prepare (query, specific = true) {
  // Normal query
  let fnCode = ''
  let v = false
  let onlyFn = null
  ...
  if(specific){
    const fn = new Function('query', 'comparisonFunctions', 'logicalOperators', 'matchQueryPart', 'getDotValue', `return obj=>{${fnCode}}`);
    return fn(query, comparisonFunctions, logicalOperators, matchQueryPart, getDotValue)
  }

  // Optimize out unused globals
  // we can invest more time here as this prepared statement should be used many times
  const args = ['comparisonFunctions', 'logicalOperators', 'matchQueryPart', 'getDotValue']
  const argValues = [comparisonFunctions, logicalOperators, matchQueryPart, getDotValue]
  for(let i=0;i<args.length;i++){
    if(!fnCode.includes(args[i])){
      args.splice(i, 1)
  ...
  const fn = new Function(...args, `return (query, obj)=>{${fnCode}}`);
  return fn(...argValues)
}
...
function matchArray(obj, queryKey, queryValue, objValue){
  ...
  if (queryValue !== null && typeof queryValue === 'object') {
    for (const key in queryValue) {
      if (arrayComparisonFunctions[key]) { return matchQueryPart(obj, queryKey, queryValue, true); }
    }
  }
  ...
  if (typeof queryValue === 'object' && queryValue !== null && !Array.isArray(queryValue)) {
    ...
    if (startsDollar) {
      for (const key in queryValue){
        ...
        if (!operator(objValue, queryValue[key])) { return false; }
      }
      return true;
    }
  }
