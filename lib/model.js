...
function match (obj, query) {
  ...
  for (let i = 0; i < queryKeys.length; i ++) {
    ...
    if (queryKey[0] === '$') {
      ...
      if (!operator(obj, queryValue)) { return false; }
...
function matchQueryPart (obj, queryKey, queryValue, treatObjAsValue) {
  ...
  if (queryValue !== null && typeof queryValue === 'object' && !util.isRegExp(queryValue) && !Array.isArray(queryValue)) {
    ...
    if (dollarFirstChars.length > 0) {
      for (i = 0; i < keys.length; i ++) {
        const operator = comparisonFunctions[keys[i]]
        if (!operator) throw new Error("Unknown comparison function " + keys[i]);
        if (!operator(objValue, queryValue[keys[i]])) { return false; }
      }
      return true;
    }
  }
