...
HashIndex.prototype.update = function (oldDoc, newDoc) {
  const oldKey = model.getDotValue(oldDoc, this.fieldName)
  const newKey = model.getDotValue(newDoc, this.fieldName)
  if(oldKey == newKey) this.data[oldKey] = newDoc
  else{
    this.remove(oldDoc);
    try {
      this.insert(newDoc);
...
HashIndex.prototype.getAllIt = function*(){
  const data = this.data
  for(const d in data) yield data[d]
}
