  ...
  const doCompaction = () => {
    ...
    if(self.writtenCount >= minimumWritten){
      self.compactDatafile(()=>{
        if(currentCompactionTimer && currentCompactionTimer == this.autocompactionIntervalId) this.autocompactionIntervalId = setTimeout(doCompaction, realInterval);
      });
    }
  }
...
Persistence.prototype.readFileAndParse = function (filename, cb) {
  var self = this
  , count = 0
  , hasLine = true
  , tdata = []
    ...
    function (cb) {
      Persistence.ensureDirectoryExists(path.dirname(self.filename), function (err) {
        storage.ensureDatafileIntegrity(self.filename, function (err) {
          self.readFileAndParse(self.filename, function (err, treatedData) {
            if (err) { return cb(err); }
