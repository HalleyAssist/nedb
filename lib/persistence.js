...
function Persistence (options) {
  ...
  for (let _id = 1; _id < 30; _id +=7) {
    for (let j = 0; j < 10; j += 1) {
      const s = this.beforeDeserialization(this.afterSerialization({_id,j}))
      if (s._id != _id && s.j != j) {
 ...
 */
Persistence.prototype.setAutocompactionInterval = function (interval, minimumWritten = 0, minimumBytes = Number.MAX_SAFE_INTEGER) {
...
Persistence.prototype.readFileAndParse = async function (cb) {
  ...
  try {
    ...
    return cb(e, { data: tdata, indexes: indexes })
  }
  ...
  try {
    ...
    return cb(ex, { data: tdata, indexes: indexes })
  }

  // A bit lenient on corruption
  if (count > 0 && corruptItems / count > this.corruptAlertThreshold) {
  ...
  cb(null, { data: Object.values(dataById), indexes: indexes })
}
