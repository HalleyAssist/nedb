...
Datastore.prototype.getCandidates = function (query, dontExpireStaleDocs, callback) {
  ...
  for(const doc of docs){
    ...
    for(const i in this.ttlIndexes) {
      if (doc[i] !== undefined && util.isDate(doc[i]) && Date.now() > doc[i].getTime() + this.ttlIndexes[i] * 1000) {
        valid = false;
      }
    }
  ...
  async.eachSeries(expiredDocsIds, (_id, cb) => {
    this._remove({ _id: _id }, {}, function (err) {
      if (err) { return callback(err); }
      return cb();
    });
  }, function (err) {
...
Datastore.prototype.createNewId = function () {
  ...
  if (this.indexes._id.getMatchingSingle(tentativeId).length > 0) {
