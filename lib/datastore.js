var customUtils = require('./customUtils')
  ...
  , Index = require('./index')
  , NumberIndex = require('./numberIndex')
  , HashIndex = require('./hashIndex')
  , util = require('util')
...
Datastore.prototype.ensureIndex = function (options, cb) {
  ...
  let index
  if(options.hash){
    index = new HashIndex(options)
  }else{
    if(!options.sparse && NumberIndex.interested[options.type]){
      index = new NumberIndex(options)
    }else{
...
Datastore.prototype._remove = function (query, options, cb) {
  ...
  this.getCandidates(query, true, (err, candidates) => {
    ...
    if(removedDocs.length){
      this.persistence.persistNewState([{ $$deleted: removedDocs}], function (err) {
        if (err) { return callback(err); }
