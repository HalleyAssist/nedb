...
Datastore.prototype._upsert = function (query, updateQuery, options, cb) {
  ...
  cursor.limit(1)._exec((err, docs) => {
    if (err) { return callback(err); }
    if (docs.length === 1) {
      this.updateIndexes(docs[0], updateQuery)

      // Update the datafile
      this.persistence.persistNewState([updateQuery], true, function (err) {
        if (err) { return cb(err); }
        if(returnValue){
          cb(null, [model.deepCopy(updateQuery)])
        }else{
          cb(null, 1)
        }
      });
...
Datastore.prototype._update = function (query, updateQuery, options, cb) {
  ...
  this.getCandidates(query, (err, candidates) => {
    ...
    // Update the datafile
    this.persistence.persistNewState(updatedDocs, true, function (err) {
      if (err) { return callback(err); }
