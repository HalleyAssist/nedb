var customUtils = require('./customUtils')
  , model = require('./model')
  , [AsyncWaterfall, AsyncEachSeries] = [require('async/waterfall'), require('async/eachSeries')]
 ...
 */
Datastore.prototype.findUnsafe = function findUnsafe(query, projection, callback) {
...
Datastore.prototype._update = function (query, updateQuery, options, cb) {
  ...
  function (cb) {   // If upsert option is set, check whether we need to insert the doc
    ...
    var cursor = new Cursor(self, upsertQuery);
    cursor.limit(1)._exec(function (err, docs) {
      ...
      return self._insert(toBeInserted, false, function (err, newDoc) {
        if (err) { return callback(err); }
        return callback(null, 1, returnValue?model.deepCopy(newDoc):newDoc.length, true);
      });
    });
  }
    ...
    self.getCandidates(query, function (err, candidates) {
      ...
      try {
        ...
        for(const doc of candidates){
          ...
          numReplaced += 1;
          if (self.timestampData) { createdAt = doc.createdAt; }
      ...
      // Update the datafile
      self.persistence.persistNewState(updatedDocs, true, function (err) {
        if (err) { return callback(err); }
...
Datastore.prototype._removeFn = function(match, cb) {
  ...
  if(removedDocs.length){
    this.persistence.persistNewState([{ $$deleted: removedDocs}], true, function () {
      return cb(true);
    });
  }else{
    cb(true)
...
Datastore.prototype._removeById = function (id, cb) {
...
Datastore.prototype.persistRemoval = function () {
  this.executor.push({ this: this, fn: this._persistRemoval, arguments: arguments });
};
